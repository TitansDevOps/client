...
export default function Sidebar({ children }) {
  ...
  return (
    <aside className={`relative h-screen transition-all duration-300 ${expanded ? "w-64" : "w-16"}`}>
      <button
      onClick={() => setExpanded((curr) => !curr)}
      className="absolute top-2 right-2 z-10 p-1.5 rounded-lg bg-gray-50 hover:bg-gray-100"
      >
        {expanded ? <ChevronFirst style={{color:"black"}}/> : <ChevronLast style={{color:"black"}}/>}   
      </button>
        ...
        <div className="border-t flex p-3">
          ...
          >
            <div className="leading-4">
              <h4 className="font-semibold" style={{color:"black"}}>TitansDevOps</h4>
              <span className="text-xs text-gray-600">TitansDevOps@gmail.com</span>
            </div>
