...
export default function PetCard({ pet, isFeatured = false }) {
  ...
  return (
    <div className={`bg-white rounded-lg overflow-hidden shadow-md border ${isFeatured ? 'border-blue-300' : 'border-gray-200'} transition-all duration-300 ${isFeatured ? 'scale-100' : 'scale-90'}`}>
      {/* Imagen */}
      ...
      <div className="p-4">
        <div className="flex justify-between items-start mb-2">
          <div>
            <h3 className={`${isFeatured ? 'text-lg font-bold' : 'text-md font-semibold'} text-gray-800`}>{pet.name}</h3>
            <div className="flex items-center text-xs text-gray-500 mt-1">
              <MapPin className="h-3 w-3 mr-1" />
              <span>{pet.location}</span>
            </div>
          </div>
          <div className={`p-1.5 rounded-full ${pet.gender === 'male' ? 'bg-blue-100 text-blue-600' : 'bg-pink-100 text-pink-600'}`}>
            {pet.gender === 'male' ? (
